...
public:
	...
	double ObjFunction4D(double xc, double yc, double r, double mu, double Lambda, int rPow); // done
	
	Point4D ObjFunction4DGrad(double xc, double yc, double r, double mu, double Lambda,int rPow); // done
	matrix<double,4,4> ObjFunction4DHessian(double xc, double yc, double r, double mu, double Lambda, int rPow); 

	// Set image data
	void SetImage(cv::Mat mImage);
	...
	double Sigma2D(double x, double y, double xc, double yc,
		double rx, double ry, double mu, double theta ,Vector3D uVect);

	double Sigma2DDerXc(double x, double y, double xc, double yc,
		double rx, double ry, double mu, double theta ,Vector3D uVect);

	Vector3D RotateVector(Vector3D XYZVect, double theta, Vector3D uVect);
	Vector3D RotateVectorDerXc(double theta, Vector3D uVect);
	Vector3D RotateVectorDerYc(double theta, Vector3D uVect);
	Vector3D RotateVectorDerZc(double theta, Vector3D uVect);
  Vector3D RotateVectorDerAngle(Vector3D XYZVect,double theta, Vector3D uVect);

	double ObjFunction2DRot(double xc, double yc, double rx, double ry, double mu,
		double Lambda, int rPow,double theta, Vector3D uVect); // done

	Point5D ObjFunction2DRotGrad(double xc, double yc, double rx, double ry, double mu,
		double Lambda, int rPow,double theta, Vector3D uVect); // done
