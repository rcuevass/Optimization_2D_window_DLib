...
double xCoord = double(25);
double yCoord = double(25);


double boxMin = double(10);
double boxMax = double(80);
...
*/
{
    ...
    //return 100.0*pow(y - x*x,2) + pow(1 - x,2);
		...
		return -1.0*getFunct.ObjFunction2D(x,y,r,mu);
...
!*/
{
    ...
    column_vector res(2);
		...
		gradVal = getFunct.ObjFunction2DGrad(x,y,r,mu);
...
matrix<double> rosen_hessian (const column_vector& m)
{
    ...
    matrix<double> res(2,2);
		...
		HessValues = getFunct.ObjFunction2DHessian(x,y,r,mu);
    ...
    //res(1,1) = 200;                 // second derivative with respect to y
		...
		res(0,0) = -1.0*HessValues[0]; // second derivative with respect to Xc
		res(1,0) = res(0,1) = -1.0*HessValues[1];  // derivative with respect to Xc and Yc
		res(1,1) = -1.0*HessValues[1]; // second derivative with respect to Yc
...
int main()
{
    ...
    try
    {
        ...
        //                         rosen, rosen_derivative, starting_point, 0.1, 0.8);
				...
				starting_point = xCoord, yCoord; // Start with a valid point inside the constraint box.
				find_min_box_constrained(lbfgs_search_strategy(10),
																 objective_delta_stop_strategy(1e-9),  
        ...
        //cout << endl << "constrained rosen solution: \n" << starting_point << endl;
				...
				find_min_box_constrained(bfgs_search_strategy(),  
					objective_delta_stop_strategy(1e-9),  
					rosen, derivative(rosen), starting_point, 20.0, 70.0);
				cout << endl << "constrained rosen solution: \n" << starting_point << endl;
